...
import type { EdgeValuedGraph } from '@rimbu/graph';
import type { ValuedGraphElement, ValuedLink } from '@rimbu/graph/custom';
import { Stream, StreamSource } from '@rimbu/stream';

function symmetric<N, V>(source: StreamSource<ValuedLink<N, V>>): [N, N, V][] {
  return Stream.from(source)
    .concat(
      Stream.from(source).collect(([n1, n2, v], _, skip) => {
...
export function runGraphTestsWith(
  ...
  describe(`${name} methods`, () => {
    ...
    it('getConnectionsFrom', () => {
      ...
      expect(new Set(graph3.getConnectionsFrom('a'))).toEqual(
        new Set([
          ['b', 1],
          ['c', 3],
        ])
  ...
  describe(`${name}.Builder`, () => {
    function forEachBuilder(
      f: (builder: EdgeValuedGraph.Builder<string, number>) => void
    ) {
    ...
    it('modifyAt', () => {
      ...
      expect(b.getValue('b', 'a')).toBe(undefined);

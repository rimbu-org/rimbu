import { Action, Actor, MachineSlice } from 'main/index.mjs';
...
describe('MachineSlice', () => {
  it('can work without states', () => {
    ...
    const slice = MachineSlice.create({
      states: undefined,
      actions,
      context: { count: 0 },
      transitions: (on) => [
        ...
        active: {
          ...
          transitions: (on) => [
            ...
            on(actions.deactivate, {
              update: (context) => ({ ...context, active: true }),
              to: '.active',
            }),
          ],
        },
    ...
    const slice = MachineSlice.create(
      {
        ...
        states: {
          a: {
            initState: 'b',
            enter: { update: (context) => ({ ...context, level1: 'enter a' }) },
            states: {
              b: {
                states: undefined,
                enter: {
                  update: (context) => ({ ...context, level2: 'enter a.b' }),
                },
                transitions: (on) => [on(actions.action, { to: '.a.c' })],
              },
              c: {
                states: undefined,
                enter: {
                  update: (context) => ({ ...context, level2: 'enter a.c' }),
                },
                transitions: (on) => [on(actions.action, { to: '.d' })],
              },
            },
          ...
          d: {
            enter: { update: (context) => ({ ...context, level1: 'enter d' }) },
            states: undefined,
            transitions: (on) => [on(actions.action, { to: '.a.b' })],
          },
        },
  ...
  it('correctly performs exit actions', () => {
    ...
    const slice = MachineSlice.create({
      ...
      states: {
        a: {
          initState: 'b',
          exit: { update: (context) => ({ ...context, level1: 'exit a' }) },
          states: {
            b: {
              states: undefined,
              exit: {
                update: (context) => ({ ...context, level2: 'exit a.b' }),
              },
              transitions: (on) => [on(actions.action, { to: '.a.c' })],
            },
            c: {
              states: undefined,
              exit: {
                update: (context) => ({ ...context, level2: 'exit a.c' }),
              },
              transitions: (on) => [on(actions.action, { to: '.d' })],
            },
          },
        ...
        d: {
          exit: { update: (context) => ({ ...context, level1: 'exit d' }) },
          states: undefined,
          transitions: (on) => [on(actions.action, { to: '.a.b' })],
        },
      },
  ...
  it('correctly applies most specific handler with guards', () => {
    ...
    const slice = MachineSlice.create({
      ...
      transitions: (on) => [
        on(actions.notify, {
          to: '.a',
          update: (context) => ({ ...context, outer: true }),
        }),
      ],
    });
      ...
      states: {
        a: {
          ...
          enter: {
            ...
            update: (context) => ({ ...context, value: -5 }),
          },
          exit: {
            guard: (context) => context.value > 0,
            update: (context) => ({ ...context, value: -7 }),
          },
        },
        b: {
          states: undefined,
  ...
  it('executes enter actions on initial state', () => {
    const actions = {
      action: Action.create(),
    };
    ...
    const slice = MachineSlice.create({
      ...
      states: {
        a: {
          states: undefined,
          enter: { update: (context) => ({ ...context, value: 1 }) },
          transitions: (on) => [on(actions.action, { to: '.b' })],
        },
        b: {
          states: undefined,
          enter: { update: (context) => ({ ...context, value: 2 }) },
          transitions: (on) => [on(actions.action, { to: '.a' })],
        },
      },
    });

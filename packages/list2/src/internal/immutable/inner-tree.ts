...
export class InnerTree<T, C extends Self<Block<T>, C>> implements Inner<T, C> {
	...
	#createNormalized(
		...
		if (null === middle) {
			const nrChildren = left._nrChildren + right._nrChildren;

			if (nrChildren <= 2 * this.context.maxBlockSize) {
	...
	dropLastChild(): [Inner<T, C> | null, C] {
		...
		if (null === newRight) {
			...
			const newSelf = this.#createNormalized(
				undefined,
				toRight,
				newMiddle,
	...
	dropInternal(
		...
		if (null === this.middle) {
			...
			if (!Int.isAtLeastOne(middleAmount)) {
				// middleAmount === 0: drop exactly left.size, boundary at right start

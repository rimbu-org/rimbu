...
import { FastIterable, Streamable, StreamSource } from './internal.ts';
...
export interface Stream<T> extends FastIterable<T>, Streamable<T> {
   ...
   * @param collectFun - a function taking the parameters below and returning a new element or a skip token
   * * value: the next element
   ...
   * @example
   * function compareLength(a: string, b: string): number { return b.length - a.length };
   * Stream.of('abc', 'a', 'ab').maxBy(compareLength)   // => 'a'
...
export const Stream = Constructors;

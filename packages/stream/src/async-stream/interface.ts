...
export interface AsyncStream<T>
   ...
   * @param flatMapFun - a potentially asynchronous function receiving the inputs described below and returning a `StreamSource` of new elements
   * * value: the next element
   * * index: the index of the element
   * * halt: a function that, if called, ensures that no new elements are passed
   * @note O(1)
   ...
   * @param collectFun - a potentially asynchronous function taking the parameters below and returning a new element or a skip token
   * * value: the next element
   ...
   * @example
   * function compareLength(a: string, b: string): number { return b.length - a.length };
   * await AsyncStream.of('abc', 'a', 'ab').maxBy(compareLength)   // => 'a'
   ...
   * @param options - (optional) object specifying the following properties
   ...
   * `options.end`.
   * @param options - object specifying the following properties
   ...
   * @param next - a potentially asynchronous function taking the parameters below and returning the new result/state value

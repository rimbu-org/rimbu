...
import { ReducerBase } from './reducer-base.mjs';
import { InvalidCombineShapeError } from './reducer-errors.mjs';
import type { ReducerFactory } from './reducer-factory.mjs';
...
export const reducerFactoryModule = Module.create<ReducerFactory>((mod) => ({
  ...
  toJSObject: Module.factory(<
    ...
    V,
  >(): Reducer<readonly [K, V], Record<K, V>> => {
    return mod.create(
      () => ({}) as Record<K, V>,
      (state, entry) => {

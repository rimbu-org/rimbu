...
describe('Match.all', () => {
  ...
  it('matches simple', () => {
    ...
    expect(Match.all([1, 2, 3])(Literal([1, 2, 3]))).toBe(false);
  });

  it('matches undefined', () => {
    expect(Match.all(undefined)(Literal(undefined))).toBe(true);
    expect(Match.all(undefined as number | undefined)(1)).toBe(false);
    expect(Match.all(undefined as number | undefined)(Literal(1))).toBe(false);

    expect(() => Match.all({ v: undefined })({ v: undefined })).toThrow();
    expect(Match.all({ v: undefined })({ v: Literal(undefined) })).toBe(true);
    expect(
  ...
  it('handles array', () => {
    expect(Match.all({ s: [1] })({ s: Literal([]) })).toBe(false);
    expect(Match.all({ s: [1] })({ s: Literal([1]) })).toBe(false);
  ...
  it('matches function', () => {
    ...
    expect(Match.all({ a: f })({ a: Literal(f) })).toBe(true);
    expect(Match.all({ a: f })({ a: Literal(() => 1) })).toBe(false);
  });

  it('matches list', () => {
    expect(Match.all({ s: List.of(1) })({ s: Literal(List.of(1)) })).toBe(
      false
    );
    expect(Match.all({ s: List.of(1) })({ s: Literal(List.of(1, 2)) })).toBe(
      false
    );
    expect(Match.all({ s: List.of(1, 2) })({ s: Literal(List.of(1)) })).toBe(
      false
    );
  });
  ...
  it('matches map', () => {
    ...
    expect(
      Match.all(obj)({
        personAge: Literal(SortedMap.of(['Jim', 25], ['Bob', 56])),
      })
    ).toBe(false);
  });
});

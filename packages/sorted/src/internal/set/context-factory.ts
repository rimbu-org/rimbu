	...
	implements SortedSet.Advanced.ContextApi<UT, SortedSet.Advanced.Family<UT>>
{
	...
	constructor(
		...
		readonly getDefaultInstance: () => SortedSetContext<any> = () =>
			this as unknown as SortedSetContext<any>,
	) {
		super();
	...
	isNonEmptyInstance<T extends UT>(
		source: unknown,
	): source is SortedSet.NonEmpty<T> {
	...
	empty = <T extends UT>(): SortedSet<T> => {
		if (undefined === this.#empty) {
			this.#empty = Object.freeze(
				new SortedSetEmpty<T>(this as unknown as SortedSetContext<T>),
			) as unknown as SortedSet<UT>;
		}
	...
	createBuilder<T extends UT>(source?: SortedSet<T>): SortedSet.Builder<T> {
		return new SortedSetBuilder<T>(
			this as unknown as SortedSetContext<T>,
			source as SortedSet<T>,
		);
	}
	...
	return {
		getDefinition: () =>
			({}) as unknown as ReturnType<
				Module<SortedSetContext<UT>>['getDefinition']
			>,
		build: () => context,

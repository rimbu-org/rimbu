...
> extends FastIterable<[R, C, V]> {
   ...
   */
  readonly rowMap: CB.WithRow<Tp, R, C, V>['rowMap'];
  /**
   ...
   * }
   */
  nonEmpty(): this is CB.WithRow<Tp, R, C, V>['nonEmpty'];
  /**
   ...
   */
  assumeNonEmpty(): CB.WithRow<Tp, R, C, V>['nonEmpty'];
  /**
   ...
   */
  getRow<UR = R>(row: RelatedTo<R, UR>): CB.WithRow<Tp, R, C, V>['row'];
  /**
   ...
   */
  removeRow<UR = R>(row: RelatedTo<R, UR>): CB.WithRow<Tp, R, C, V>['normal'];
  /**
  ...
  removeAndGet<UR = R, UC = C>(
    row: RelatedTo<R, UR>,
    column: RelatedTo<C, UC>
  ): [CB.WithRow<Tp, R, C, V>['normal'], V] | undefined;
  /**
  ...
  removeRowAndGet<UR = R>(
    ...
    | [
        CB.WithRow<Tp, R, C, V>['normal'],
        CB.WithRow<Tp, R, C, V>['rowNonEmpty']
      ]
    | undefined;
  /**
  ...
  filterRows(
    pred: (
      entry: readonly [R, CB.WithRow<Tp, R, C, V>['rowNonEmpty']],
      index: number,
      halt: () => void
    ) => boolean
  ): CB.WithRow<Tp, R, C, V>['normal'];
...
export namespace VariantTableBase {
  ...
  > extends VariantTableBase<R, C, V, Tp>,
     ...
     */
    readonly rowMap: CB.WithRow<Tp, R, C, V>['rowMapNonEmpty'];
    /**
     ...
     */
    asNormal(): CB.WithRow<Tp, R, C, V>['normal'];
    /**
...
> extends VariantTableBase<R, C, V, Tp> {
   ...
   */
  readonly context: CB.WithRow<Tp, R, C, V>['context'];
  /**
   ...
   * to generate new values. If a function returning the token argument is given, no new entry is created.
...
export namespace TableBase {
  ...
  > {
     ...
     */
    readonly rowContext: (Tp & CB.Row<UR, UC, any>)['rowContext'];
    /**
     * The context used for the internal column map instances.
     */
    readonly columnContext: (Tp & CB.Row<UR, UC, any>)['columnContext'];
  ...
  > {
     ...
     */
    build(): CB.WithRow<Tp, R, C, V>['normal'];
    /**
     * Returns an immutable collection instance containing the entries in this builder.

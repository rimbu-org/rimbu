...
`hasValue` (the 3rd element) now means **"the relevant key/value was present, so a value is returned"** — it does NOT indicate whether the collection changed. When an operation is a no-op (e.g. an `update` that yields the same value), `hasValue` is still `true` and the previous value is returned, while the `result` is the unchanged collection. To detect whether the collection actually changed, compare `result[0] === this` (immutable structures return `this` on a no-op).

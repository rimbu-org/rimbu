...
- **Overload order for `NonEmpty` variants** — when a method on a `NonEmpty` variant has two overloads (one returning the `normal`/`possibly empty` type, one returning the `NonEmpty` type), the `NonEmpty`-returning overload MUST be declared **first**. TypeScript selects the first overload whose parameters match the call, so if the `normal` overload comes first, a callback that returns a `StreamSource.NonEmpty` is matched by the `normal` overload and the precise `NonEmpty` return type is lost. Example: `transform` on `List.NonEmpty` / `MultiMap.NonEmpty` declares the `StreamSource.NonEmpty` overload before the plain `StreamSource` overload for exactly this reason.

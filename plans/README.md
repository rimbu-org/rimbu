...
| `@rimbu/proximity` | [proximity.md](proximity.md) | Root re-exports; `distance-function`/`key-matching` → `src/public/`; **remove `"./*"` leak.** |
| `@rimbu/table` | [table.md](table.md) | Root re-exports; `hash-row`/`sorted-row` variant factories → `src/advanced/`; **remove `"./*"` leak.** |

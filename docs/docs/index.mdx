...
- [`List`](collections/list): a block-based random-access structure akin to a `Vector`
- [Maps](collections/map): `HashMap`, `SortedMap`, `OrderedMap`
- [Sets](collections/set): `HashSet`, `SortedSet` `OrderedSet`
- [`Stream`](collections/stream): a way to create and convert streams of data

It also contains less commonly used, but very useful collections, like [`MultiMap`](collections/multimap), [`BiMultiMap`](collections/bimultimap), [`MultiSet`](collections/multiset), [`Table`](collections/table), [`Graph`](collections/graph).

### Immutable object utilities

Next to collections, there are also tools to treat plain JS objects as immutable objects, most notable are [`Match`](.), [`Patch`](.), and [`Path`](.). The

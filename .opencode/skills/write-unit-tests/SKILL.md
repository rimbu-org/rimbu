...
1. Resolve target: single package `<pkg>` (e.g. `packages/hashed`) is default. If `--workspace` is passed, expand to all 23 published packages with `package.json`. Require `<pkg>` if no `--workspace`. Be single-package-scoped and idempotent (Q6).
